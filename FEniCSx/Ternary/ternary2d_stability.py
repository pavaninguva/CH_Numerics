...
# cahn_hilliard_analytical(ic_fun_a, ic_fun_b, 6,6,6,1,1,1,20,100,0.2,0.1,False,True)
...
def find_critical_dt(sim_func,ic_fun_a, ic_fun_b, chi, N1, N2, N3, stride, tend, deltax, dt_start=0.5, dt_min=1e-4):
    dt = dt_start
    while dt >= dt_min:
        try:
            # Run the simulation with current dt.
            # Set return_data=False to keep things fast.
            sim_func(ic_fun_a,ic_fun_b,chi,chi,chi,N1,N2,N3,stride,tend,deltax,dt,return_data=False,return_vtk=False)
...
def worker(params, sim_func):
    chi, deltax, dt_start, dt_min, ic_fun_a, ic_fun_b, N1, N2, N3, stride = params
    tend = get_tend_for_chi(chi)
    critical_dt = find_critical_dt(sim_func, ic_fun_a, ic_fun_b, chi, N1, N2, N3, stride, tend, deltax, dt_start, dt_min)
...
def run_parameter_study_mp(chi_values, deltax_values, dt_start, dt_min,
                           ic_fun_a,ic_fun_b, N1, N2, N3, stride, num_workers, csv_filename, sim_func):
    """
    ...
    for chi in chi_values:
        for deltax in deltax_values:
            ...
            if key not in existing_results:
                param_list.append((chi, deltax, dt_start, dt_min, ic_fun_a,ic_fun_b, N1, N2, N3, stride))
...
if __name__ == '__main__':
    ...
    run_parameter_study_mp(
        chi_values,deltax_values,
        dt_start=0.5,dt_min=1e-4,
        ic_fun_a=ic_fun_a, ic_fun_b=ic_fun_b,
        N1=N1, N2=N2, N3=N3,stride=stride,
        num_workers=4, csv_filename=analytical_csv,
        sim_func=cahn_hilliard_analytical
    )

    plot_dual_heatmaps_pcolormesh(analytical_csv,analytical_csv)
